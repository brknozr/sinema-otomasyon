...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Kullanıcı adı ve şifre alanlarının doldurulduğunu kontrol etme
    if(empty($uye_adi) || empty($uye_sifre)) {
        echo "Lütfen kullanıcı adı ve şifre alanlarını doldurun.";
    } else {
        // Veritabanında yöneticiyi kontrol et
        $sql = "SELECT * FROM uyeler WHERE uye_adi = '$uye_adi'";
        $result = mysqli_query($conn, $sql);
        $count = mysqli_num_rows($result);

        // Eğer yönetici bulunamazsa
        if($count == 0) {
            echo "Bu kullanıcı adıyla kayıtlı bir yönetici bulunamadı.";
        } else {
            // Yönetici bulunduysa, şifre kontrolü yap
            ...
            if($uye_sifre != $row['uye_sifre']) {
                ...
                header("Location:admin_islem.php");
                exit(); // Bu satırı ekledim, yönlendirme sonrasında kodun devam etmemesi için.
            }
        }
    }
}
...
<div class="tableouter">
    ...
    <a href="index.php"><button type="button" class="sub">Ana Sayfa</button></a>
</div>
</body>
</html>
